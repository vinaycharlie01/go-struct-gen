...
class JSONToGoConverter:
    ...
    def parse_scope(self, scope, depth=0):
        if isinstance(scope, dict):
            ...
            if slice_type == "struct":
                self.parse_struct(depth + 1, self.inner_tabs,scope[0])
    ...
    def compare_object_keys(self, item_a_keys, item_b_keys):
        return set(item_a_keys) == set(item_b_keys)
    ...
    def is_datetime_string(self, string):
        try:
            ...
            return False
# # Example Usage        
json_input='{ "filter":{ "sitereference":[ { "value":"shshs" } ], "transactionreference":[ { "value":"3-64-25205" } ] } }'
typename = "EventData1"

converter = JSONToGoConverter(json_input, typename, flatten=False, example=False, all_omitempty=True,bson=False,bson_omitempty=False)
